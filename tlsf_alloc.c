...
static inline blk_hdr_t* split_blk(blk_hdr_t* blk, size_t split_size) {
    ...
    leftover_blk->prev_size = blk->size;
    return leftover_blk;
}

static inline blk_hdr_t* coalesce_blk(blk_hdr_t* blk) {
    blk_hdr_t* prev_blk = get_prev_block(blk);
    if (prev_blk != NULL && prev_blk->size == blk->prev_size) {
    ...
    if (next_blk != NULL && next_blk->size > 0 && next_blk->prev_size == blk->size) {
...
void tlsf_add_pool_lk(void* mem_start, size_t mem_size, int fl_index) {
    if (tlsf_allocator.fl_locks[fl_index]) tlsf_allocator.fl_locks[fl_index](fl_index);
    tlsf_add_pool_internal(mem_start, mem_size);
    if (tlsf_allocator.fl_unlocks[fl_index]) tlsf_allocator.fl_unlocks[fl_index](fl_index);
}
...
void tlsf_free_lk(void* p) {
    ...
    if (tlsf_allocator.fl_locks[fl]) tlsf_allocator.fl_locks[fl](fl);
    tlsf_free(p);
    if (tlsf_allocator.fl_unlocks[fl]) tlsf_allocator.fl_unlocks[fl](fl);
}
