...
static inline int old_tlsf_find_free_ls(size_t size, int* fl, int* sl) {
    if (size == 0)
        return -1;
...
static void old_tlsf_add_blk_to_free_ls(blk_hdr_t* blk) {
    int fl, sl;
    old_tlsf_find_free_ls(blk->size, &fl, &sl);
...
static void old_tlsf_rm_blk_from_free_ls(blk_hdr_t* blk) {
    int fl, sl;
    old_tlsf_find_free_ls(blk->size, &fl, &sl);
...
static inline blk_hdr_t* old_tlsf_split_blk(blk_hdr_t* blk, size_t split_size) {
...
static inline blk_hdr_t* old_tlsf_coalesce_blk(blk_hdr_t* blk) {
    blk_hdr_t* prev_blk = blk->prev_block;
    if (prev_blk != NULL && (size_t)prev_blk + prev_blk->size == (size_t)blk) {
        old_tlsf_rm_blk_from_free_ls(prev_blk);
    ...
    if (next_blk != NULL && (size_t)blk + blk->size == (size_t)next_blk) {
        old_tlsf_rm_blk_from_free_ls(next_blk);
...
static void old_tlsf_add_pool_internal(void* mem_start, size_t mem_size) {
    ...
    old_tlsf_add_blk_to_free_ls(initial_blk);
}

void old_tlsf_add_pool(void* mem_start, size_t mem_size) {
    old_tlsf_add_pool_internal(mem_start, mem_size);
}
...
void old_tlsf_init() {
    ...
    old_tlsf_add_pool_internal(memory_pool, POOL_SIZE_BYTES);
}
...
void* old_tlsf_alloc(size_t size) {
    ...
    int fl, sl;
    old_tlsf_find_free_ls(adjusted_size, &fl, &sl);
    ...
    old_tlsf_rm_blk_from_free_ls(blk);

    if (blk->size > adjusted_size) {
        blk_hdr_t* leftover_blk = old_tlsf_split_blk(blk, adjusted_size);
        old_tlsf_add_blk_to_free_ls(leftover_blk);
    }
...
void* old_tlsf_alloc_ali(size_t size, size_t alignment) {
    ...
    int fl, sl;
    old_tlsf_find_free_ls(required_size, &fl, &sl);
    ...
    if (blk->size > aligned_blk_size) {
        blk_hdr_t* leftover_blk = old_tlsf_split_blk(blk, aligned_blk_size);
        old_tlsf_add_blk_to_free_ls(leftover_blk);
    }
...
void old_tlsf_free(void* p) {
    ...
    blk_hdr_t* merged_blk = old_tlsf_coalesce_blk(blk);
    old_tlsf_add_blk_to_free_ls(merged_blk);
}
...
void* old_tlsf_alloc_ali_lk(size_t size, size_t alignment) {
    ...
    void* p = old_tlsf_alloc_ali(size, alignment);
